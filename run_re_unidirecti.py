...
class ACEDataset(Dataset):
    def __init__(self, tokenizer, args=None, evaluate=False, do_test=False, max_pair_length=None):
        ...
        # self.eval_pred_ent = args.eval_pred_ent
        self.global_predicted_ners = {}
        self.initialize()
 
    def initialize(self, eval_cands=None):
        ...
        for l_idx, line in enumerate(f):
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                if eval_cands:
                    if (l_idx, n) in eval_cands:
                        entities = eval_cands[(l_idx, n)]
                    else:
                        continue
                else:
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    ner_ori_p = ner_ori_cor / ner_tot_pred if ner_tot_pred > 0 else 0 
...
def main():
    ...
    parser.add_argument("--test_file",  default="ent_pred_test.json", type=str)
    parser.add_argument('--max_pair_length', type=int, default=128,  help="")
    parser.add_argument("--eval_pred_ent", action='store_true', help="")
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test')==-1:
        create_exp_dir(args.output_dir, scripts_to_save=['run_re_unidrect.py', 'transformers/src/transformers/modeling_bert.py', 'transformers/src/transformers/modeling_albert.py'])
    ...
    if args.do_train:
        ...
        if args.lminit:
            ...
            word_embeddings[subs].copy_(word_embeddings[mask_id])   # subject
            word_embeddings[sube].copy_(word_embeddings[subject_id])    

            word_embeddings[objs].copy_(word_embeddings[mask_id])   # object
