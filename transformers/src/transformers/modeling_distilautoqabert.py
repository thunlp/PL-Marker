...
class Transformer(nn.Module):
    ...
    def forward(self, x, attn_mask=None, head_mask=None, copy_rate=None, tokens_prob=None):
        ...
        tot_idxs = torch.arange(num_items).unsqueeze(0).repeat(bsz, 1)
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if (i in [1, mid]) and (num_items > mini):
                ...
                if copy_rate is None:   # for testing
                ...
                if copy_rate is None:   # for testing
                    sorted_porb, indices = torch.sort(caiyang_prob, descending=True)
                    top_indices = indices[:, :num_items_to_keep]
                    del_indices = indices[:, num_items_to_keep:]
                    if final_hidden_states is None:
                        final_hidden_states = hidden_states                        
                    else:
                        tot_del_indices = tot_idxs[argbsz, del_indices]
                        final_hidden_states[argbsz, tot_del_indices] = hidden_states[argbsz, del_indices] 
                    tot_idxs = tot_idxs[argbsz, top_indices]
                else:
                    with torch.no_grad():
                        ...
                        del_state = hidden_states * del_mask.unsqueeze(-1)   
                ...
                seq_mask = seq_mask[argbsz, top_indices]
                hidden_states = hidden_states[argbsz, top_indices]
        ...
        if copy_rate is None:   # for testing
            final_hidden_states[argbsz, tot_idxs] = hidden_states
        else:
