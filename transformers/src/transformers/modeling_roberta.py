...
class RobertaForSpanNER(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
...
class RobertaForSpanMarkerNER(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
...
class RobertaOnlyLMHeadTransform(nn.Module):
    """Roberta Head for masked language modeling."""
...
class RobertaForLeftLMNER(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
        ...
        self.lm_head = RobertaOnlyLMHeadTransform(config)
    ...
    def forward(
        ...
        feature_vector = self.lm_head(e1_hidden_states)
        ner_prediction_scores = self.ner_classifier(feature_vector)
