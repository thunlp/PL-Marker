...
class BertEncoder(nn.Module):
    ...
    def forward(
        ...
        loss = torch.sum(loss, dim=-1) / torch.sum(seq_mask, dim=-1)
        outputs = (hidden_states, loss.mean())
        # if self.output_hidden_states:
...
class InitBertModel(BertPreTrainedModel):
...
class InitBertForinit(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.hidden_size = config.hidden_size

        self.bert = InitBertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class InitBertForRace(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.hidden_size = config.hidden_size
...
class InitBertForNormalRace(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.hidden_size = config.hidden_size
